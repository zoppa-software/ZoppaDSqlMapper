...
Public Module ZoppaDSqlReplace
    ...
    Private ReadOnly _logger As New Lazy(Of ILogger)(
        Function()
            If _provider IsNot Nothing Then
                Return _provider.GetService(Of ILoggerFactory)()?.CreateLogger("ZoppaDSqlReplace")
            ElseIf _logFactory IsNot Nothing Then
                Return _logFactory.CreateLogger("ZoppaDSqlReplace")
            Else
                Return Nothing
            End If
        End Function
...
#End Region
    ...
    <Extension>
    Public Function Replase(sqlQuery As String,
                            Optional parameter As Object = Nothing) As String
        Using scope = _logger.Value?.BeginScope(NameOf(Replase))
            Try
                Logger.Value?.LogDebug("compile sql : {sqlQuery}", sqlQuery)
                LoggingParameter(parameter)

                ' トークンリストを取得
                Dim tokens = GetNewOrHistory(
                    sqlQuery,
                    Function(query)
                        ' クエリをトークンに分割
                        Dim splited = LexicalAnalysis.SplitQueryToken(query)
                        LoggingTokens(splited)

                        ' 改行単位で不要な空白トークンを削除
                        Return LexicalAnalysis.RemoveSpaceCrLf(splited)
                    End Function
                )
